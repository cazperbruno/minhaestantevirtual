...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const results = await Promise.all([
      ...
      sb.from("story_views").select("*").eq("viewer_id", uid),
    ...
    const [
      profile, library, notes, reviews, reviewComments, reviewLikes,
      recommendations, recommendationComments, recommendationLikes, follows,
      achievements, goals, xpHistory, streaks, challenges, interactions,
      notifications, pushSubscriptions, activities, activityComments,
      activityLikes, loans, trades, tradeMatches, purchaseOffers, invites,
      inviteRedemptions, clubMembers, clubMessages, clubReactions,
      clubNominations, clubVotes, buddyParticipants, buddyMessages, stories,
      storyViews, appEvents, auditEvents, automationRuns,
    ] = results;

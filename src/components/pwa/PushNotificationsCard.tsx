...
/**
 * UI única para notificações. Web usa VAPID/PWA; Android/iOS exibem o estado
 * nativo até o adapter APNs/FCM ser ativado.
 */
export function PushNotificationsCard() {
  const { state, busy, supported, native, platform, subscribe, unsubscribe } = usePushNotifications();

  if (native && state === "native-pending") {
    return (
      <div className="glass rounded-2xl p-5 flex items-center gap-4">
        ...
        <div className="min-w-0">
          <p className="font-semibold text-sm leading-tight">Notificações nativas</p>
          <p className="text-xs text-muted-foreground mt-0.5 leading-snug">
            {platform === "ios"
              ? "O Readify usará APNs no iPhone/iPad."
              : "O Readify usará notificações nativas no Android."}
          </p>
        </div>
      </div>
    );
